...
const formatTimes = (event) => {
  ...
  if (event.duration !== range) additionalInfo.push(`self ${formatDuration(event.duration, chalk.bold)}`);
...
const formatEvent = (event) => {
  ...
  switch (event.name) {
    case 'webpack-compilation':
      head = `${chalk.bold(`${event.tags.name} compilation`)} ${formatTimes(event)}`;
      break;
    ...
    case 'webpack-invalidated-server':
      ...
      break;
    case 'add-entry':
      head = `${chalk.blueBright('entry')} ${formatFilename(event.tags.request)}`;
      break;
    case 'hot-reloader':
    ...
    default:
      if (event.name.startsWith('build-module-')) {
        ...
        head = `${chalk.magentaBright('module')} ${
          packageName
            ? `${chalk.bold.cyan(packageName)} (${formatFilename(event.tags.name)}${
